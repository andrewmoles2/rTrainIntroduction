...
starwars_avg <- function(summary_var, group_var = species) {

  dplyr::starwars %>%
  group_by({{group_var}}) %>%
  summarise(average := mean({{summary_var}}, na.rm = TRUE)) %>%
  arrange(desc(average))
}

starwars_avg(height, species)
starwars_avg(mass, species)
starwars_avg(birth_year, species)
```
...
```{r}
my_select <- function(...) {
  select(dplyr::starwars, ...)
}

my_select(name, birth_year)
```


Using both together can be very useful, and can give us lots of flexibility. Take our function we made earlier, we can adapt it so we can have either one or several grouping arguments. 
...
starwars_avg <- function(summary_var,...) {
...
starwars_avg(mass, species)
starwars_avg(mass, species, eye_color)
```
