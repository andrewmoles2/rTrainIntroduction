...
You should see a text divided with slashes  `/`. If you're on Windows it should start with `C:/...` while if you're on Mac you should see `/Users/<your username>/...`.
...
#Your code here
knitr::include_graphics("/Users/MOLES/Downloads/r_fundamentals_5.png")
```
...
# Creating new directories and moving them around

Before now, you probably have moved your files around your computer using your mouse (we call it `drag and drop`). Although this might have worked for you up until now, programmers usually create and move files from inside the code. Why? Imagine that you are working in a project, with multiple other analysts, where you are using +100 datasets and were tasked with cleaning some data. Firstly, such large data bases would probably be divided into different folders so that data is easier to find. If you are only using `drag and drop` any time you clean a new dataset, you would need to move the file manually to the working directory, than manually move the output you've saved to the appropriate folder. On top of that imagine now that another analyst tries using your code (as often happens in team projects). You would have to send them instructions on exactly what files to move and where. Lot's of unnecessary work and weird stares from your teammates.
...
**IMPORTANT WARNING: Make sure you check trice before deleting anything from inside the code as unlike deleting with a mouse cursor, deleted files DO NOT land in the bin. Additionally, if you ever search online for deleting files, you might stumble upon `unlink(<directory>,recursive=TRUE)` command. Unlike file.remove() unlink will not return a warning when a folder is empty but will instead delete everything inside it. If you set an argument force=TRUE, you can even make it delete the R markdown file you are using. For this very reason we advice against ever using unlink(), and instead taking time to remove files one by one. On this very rare exception, more code is better.**
