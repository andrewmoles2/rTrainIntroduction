...
# calculate avg exemptions
measles$avg_exempt <- apply(measles[, c("xmed", "xper")],
                              MARGIN = 1,
                              mean, na.rm = TRUE)

# summary of useful cols
measles_summary <- apply(measles[ ,c("mmr", "overall", "xmed", "xper", "avg_exempt")], 
                    MARGIN = 2,
                    function(x) c(median = median(x, na.rm = TRUE),
                                  mean = mean(x, na.rm = TRUE),
...
# lapply with range
lapply(measles[, c("mmr", "overall", "xmed", "xper", "avg_exempt")],
       range, na.rm = TRUE)

# sapply with range and quantiles
sapply(measles[, c("mmr", "overall", "xmed", "xper", "avg_exempt")],
       function(x) c(range = range(x, na.rm = TRUE),
                     quantiles = quantile(x, na.rm = TRUE)))

# convert character to factor
char <- sapply(measles, is.character)

measles[, char == TRUE] <- lapply(measles[, char == TRUE],
                                  as.factor)
...
tapply(measles$mmr, factor(measles$state), median, na.rm = TRUE)

tapply(measles$mmr, 
       list(factor(measles$year), factor(measles$type)),
       median, na.rm = TRUE)
